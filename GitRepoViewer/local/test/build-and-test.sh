...
ANDROID_CODE=$(find ${ANDROID_CODE_DIR}/se/juneday/gitrepoviewer/ -name "*.java" | grep -v MainActivity | grep -v Volley)

TEST_CODE_DIR=test-code
TEST_CODE=$(find ${TEST_CODE_DIR} -name "*.java")
TEST_CLASSES=$(find test-code/se -name "*.java" | sed -e "s,$TEST_CODE_DIR[/]*,,g" -e 's,\.java,,g' -e 's,\/,\.,g' )
